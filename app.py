...
def compass_map_html(image_bytes):
    import base64
    template='<!doctype html><html lang="ja"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>\n*{box-sizing:border-box}body{margin:0;font:14px system-ui,sans-serif;color:#172033;background:#fff}button{padding:10px 12px;min-height:44px;border:1px solid #cbd5e1;border-radius:9px;background:#f1f5f9;color:#172033;font:inherit}button:disabled{opacity:.45}#enable,#calibrate{background:#175cd3;color:white}nav{display:flex;gap:6px;flex-wrap:wrap;margin:8px 0}p{margin:8px 0;line-height:1.5}#status{min-height:42px;color:#344054}#viewport{position:relative;width:100%;height:390px;overflow:hidden;background:#e9eef4;border:1px solid #cbd5e1;border-radius:10px;touch-action:none}#map{position:absolute;left:50%;top:50%;transform-origin:center;max-width:none;user-select:none;pointer-events:none}#up{position:absolute;top:8px;left:50%;transform:translateX(-50%);background:#172033;color:white;padding:5px 10px;border-radius:20px;font-size:12px;z-index:2}input{width:100%;height:32px}label{display:block}small{color:#475569}\n</style><p>① センサーを有効にする → ② 手動回転で目の前の配置に合わせる → ③ 基準にして自動回転</p>\n<nav><button id="enable">センサーを有効にする</button><button id="calibrate" disabled>基準にして自動回転</button><button id="lock" disabled>向きを固定</button></nav>\n<div id="status" role="status" aria-live="polite">スマホを縦向きで、なるべく水平に持って使ってください。</div>\n<div id="viewport"><div id="up">↑ 手動で向きを合わせてください</div><img id="map" alt="回転できる美原の島図" src="data:image/png;base64,__MAP_IMAGE__"></div>\n<nav><button id="left">↶ 90°</button><button id="right">↷ 90°</button><button id="minus">縮小 −</button><button id="plus">拡大 ＋</button><button id="fit">全体を表示</button></nav>\n<label>手動回転 <output id="degrees">0°</output><input id="angle" type="range" min="-180" max="180" value="0" aria-label="手動回転"></label>\n<small>拡大後は指で移動できます。日付・表示を変えたら向きを合わせ直してください。これは現在地表示ではありません。</small>\n<script>\nconst $=id=>document.getElementById(id),map=$(\'map\'),viewport=$(\'viewport\');\nlet angle=0,zoom=1,panX=0,panY=0,heading=null,offset=0,automatic=false,listening=false,lastEvent=0,source=null,timer=null;\nconst wrap=x=>((x+180)%360+360)%360-180;\nfunction status(s){$(\'status\').textContent=s;}\nfunction paint(){const base=Math.min(viewport.clientWidth,map.naturalWidth?390*map.naturalWidth/map.naturalHeight:viewport.clientWidth);map.style.width=base+\'px\';map.style.transform=`translate(-50%,-50%) translate(${panX}px,${panY}px) rotate(${angle}deg) scale(${zoom})`;$(\'degrees\').textContent=Math.round(wrap(angle))+\'°\';$(\'angle\').value=wrap(angle);}\nfunction freeze(){automatic=false;$(\'up\').textContent=\'↑ 向き固定\';$(\'lock\').disabled=true;}\nfunction manual(value){freeze();angle=value;paint();status(\'手動調整中。配置が合ったら「基準にして自動回転」を押してください。\');}\nfunction handleOrientation(e){\n let next,kind;\n if(Number.isFinite(e.webkitCompassHeading)){if(Number.isFinite(e.webkitCompassAccuracy)&&(e.webkitCompassAccuracy<0||e.webkitCompassAccuracy>45)){freeze();heading=null;$(\'calibrate\').disabled=true;status(\'方位が不安定です。手動回転を使うか、場所を変えて合わせ直してください。\');return;}next=e.webkitCompassHeading;kind=\'compass\';}\n else if(Number.isFinite(e.alpha)){next=360-e.alpha;kind=e.absolute?\'absolute\':\'relative\';}\n else return;\n if(source&&source!==kind){freeze();status(\'方位の読み取り方式が変わりました。基準を合わせ直してください。\');}\n source=kind;heading=next;lastEvent=Date.now();$(\'calibrate\').disabled=false;\n if(automatic){const target=offset-heading;angle+=wrap(target-angle)*.3;paint();}\n else if($(\'status\').textContent.includes(\'待っています\'))status(\'センサー取得OK。手動で島図の向きを合わせ、基準にしてください。\');\n}\n$(\'enable\').onclick=async()=>{\n freeze();heading=null;source=null;$(\'calibrate\').disabled=true;\n if(!window.isSecureContext){status(\'自動回転にはHTTPSのページが必要です。手動回転は使えます。\');return;}\n if(!window.DeviceOrientationEvent){status(\'この端末では方位センサーを利用できません。手動回転は使えます。\');return;}\n try{\n  if(typeof DeviceOrientationEvent.requestPermission===\'function\'){\n   const permission=await DeviceOrientationEvent.requestPermission();\n   if(permission!==\'granted\'){status(\'センサーの使用が許可されませんでした。手動回転は使えます。\');return;}\n  }\n  if(!listening){window.addEventListener(\'deviceorientation\',handleOrientation);listening=true;}\n  lastEvent=Date.now();status(\'センサーを待っています…\');\n  if(timer)clearInterval(timer);\n  timer=setInterval(()=>{if(Date.now()-lastEvent>5000){freeze();heading=null;$(\'calibrate\').disabled=true;status(\'方位を取得できません。iPhoneはSafariで開き直してお試しください。手動回転は使えます。\');}},1500);\n }catch(e){status(\'ブラウザーでセンサーを使用できません。Safariで開き直すか、手動回転を使ってください。\');}\n};\n$(\'calibrate\').onclick=()=>{if(heading===null||Date.now()-lastEvent>5000)return;offset=angle+heading;automatic=true;$(\'up\').textContent=\'↑ 画面の上＝向いている方向\';$(\'lock\').disabled=false;status(\'自動回転中。向きがずれたら固定して、手動で合わせ直してください。\');};\n$(\'lock\').onclick=()=>{freeze();status(\'向きを固定しました。再開するときは「基準にして自動回転」を押してください。\');};\n$(\'angle\').oninput=e=>manual(Number(e.target.value));$(\'left\').onclick=()=>manual(angle-90);$(\'right\').onclick=()=>manual(angle+90);\n$(\'plus\').onclick=()=>{zoom=Math.min(10,zoom*1.4);paint();};$(\'minus\').onclick=()=>{zoom=Math.max(.35,zoom/1.4);paint();};\n$(\'fit\').onclick=()=>{panX=panY=0;const rad=angle*Math.PI/180,iw=map.naturalWidth,ih=map.naturalHeight;if(!iw||!ih)return;const base=Math.min(viewport.clientWidth/iw,390/ih);zoom=Math.min(viewport.clientWidth/(base*(Math.abs(iw*Math.cos(rad))+Math.abs(ih*Math.sin(rad)))),390/(base*(Math.abs(iw*Math.sin(rad))+Math.abs(ih*Math.cos(rad)))));paint();};\nlet pointer=null;viewport.onpointerdown=e=>{pointer={id:e.pointerId,x:e.clientX,y:e.clientY};viewport.setPointerCapture(e.pointerId);};\nviewport.onpointermove=e=>{if(!pointer||pointer.id!==e.pointerId)return;panX+=e.clientX-pointer.x;panY+=e.clientY-pointer.y;pointer={id:e.pointerId,x:e.clientX,y:e.clientY};paint();};\nviewport.onpointerup=viewport.onpointercancel=()=>{pointer=null;};\nmap.onload=paint;window.addEventListener(\'resize\',paint);\nwindow.addEventListener(\'orientationchange\',()=>{freeze();heading=null;$(\'calibrate\').disabled=true;status(\'端末の向きが変わりました。もう一度基準を合わせてください。\');});\ndocument.addEventListener(\'visibilitychange\',()=>{if(document.hidden){freeze();status(\'向きを固定しました。戻ったら基準を合わせ直してください。\');}});\nwindow.addEventListener(\'pagehide\',()=>{if(timer)clearInterval(timer);window.removeEventListener(\'deviceorientation\',handleOrientation);});paint();\n</script></html>\n'
    return template.replace("__MAP_IMAGE__",base64.b64encode(image_bytes).decode("ascii"))
...
if screen=='✨ おすすめ':
    ...
    rotate_mode=st.toggle('スマホで自動回転を試す',value=False)
    if rotate_mode:
        import streamlit.components.v1 as components
        components.html(compass_map_html(encode(focus_crop(im,focus) if focus else im)),height=780,scrolling=True)
    elif focus:
